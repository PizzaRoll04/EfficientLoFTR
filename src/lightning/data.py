...
def _safe_dist_info(trainer=None):
    """Return (world_size, rank) whether or not torch.distributed is initialized."""
    if dist.is_available() and dist.is_initialized():
        return dist.get_world_size(), dist.get_rank()
    # Fall back to Trainer info (PL2 has these), else defaults
    ws = getattr(trainer, "world_size", 1) if trainer is not None else 1
    rk = getattr(trainer, "global_rank", 0) if trainer is not None else 0
    return ws, rk


def _maybe_ddp_sampler(dataset, shuffle: bool) -> DistributedSampler | None:
    if dist.is_available() and dist.is_initialized():
...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        try:
            self.world_size, self.rank = _safe_dist_info(getattr(self, "trainer", None))
    ...
    def test_dataloader(self, *args, **kwargs):
        self_rank = getattr(self, "rank", 0)
        self_ws = getattr(self, "world_size", 1)
        logger.info(
            f"[rank:{self_rank}/{self_ws}]: Test Sampler and DataLoader re-init."
        )
        sampler = _maybe_ddp_sampler(self.test_dataset, shuffle=False)
        return DataLoader(
            self.test_dataset,
            sampler=sampler,
            **self.test_loader_params,
